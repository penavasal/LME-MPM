\message{ !name(LME-MPM.tex)}% %%%%%%%%%%%%%%%%%%%%%%% file template.tex %%%%%%%%%%%%%%%%%%%%%%%%%
% %
...
\begin{abstract}
  ...
  limitation in this paper, an explicit predictor-corrector time integration scheme has been proposed. Its superior performance mitigates the 
...
\begin{keyword}
  LME \sep MPM \sep Explicit predictor-corrector \sep Dynamic problems}
\end{keyword}
...
\ref{tab:notation-table}. Three kind of subscript or superscript are used within paper. The subscript$\qed_p$ is used to define a particle variable. While the
...
\qued$ means the dyadic operator, $(\cdot)$ and $(\colon)$ means the first and second contraction of a tensor, $\Div{\qed}$ denotes the
...
\section{Local \textit{max-ent} approximants}
...
GIMP or B-Spline MPM regarding the computational domain boundaries
...
%%% TeX-master: t
%%% End:
\message{ !name(LME-MPM.tex) !offset(-1635) }
