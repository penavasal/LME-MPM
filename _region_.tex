\message{ !name(LME-MPM.tex)}\documentclass[preprint,12pt,a4paper]{elsarticle}
...
\message{ !name(LME-MPM.tex) !offset(-3) }
...
\author{
Miguel Molinos$^a\footnote{Corresponding author: m.molinoss@outlook.es}$
...
\begin{abstract}
  ...
  integration scheme to filter spurious noises. \MP{To overcome it ,
    we propose in this paper an explicit Predictor-Corrector time integration scheme}. Its \textcolor{red}{powerful}
  ...
  cross computational grid boundaries, being \MP{caused} by
...
%%% TeX-master: t
%%% End:
\message{ !name(LME-MPM.tex) !offset(-1592) }
