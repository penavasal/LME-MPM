...
\begin{abstract}
  ...
  integration scheme has been proposed. Its superior performance
  ...
  this document adopts the \acrfull{lme} approximation schemes a
  robust substitute for the wide range of shape function in
  \acrshort{MPM}. Local \textit{max-ent} approximation may be regarded
...
(1994)~\cite{Sulsky1994} as a generalization to solids of the \acrfull{flip} method~\cite{Brackbill1986}. It popularity
...
limitation such as the \acrfull{gimp} method Bardenhagen \& Kober (2004)~\cite{Bardenhagen2004}, which has
...
been introduced in to \acrshort{mpm} framework with the Conservative Taylor
...
This document adopts the \textit{local maximum-entropy approximation
  ...
  function in \acrshort{mpm}. First introduced by Arroyo \& Ortiz
...
recently under \acrshort{mpm} framework by Wobbes {\it et
  al.}(2020)~\cite{Wobbes2020} but without exploring the benefits of
the regularization parameter \gls{LME-beta}.
...
scheme. One of the most popular is the \acrfull{igimp}
...
time integration scheme for \acrshort{mpm} is proposed. Section
...
\section{The meshfreesubsection procedure}
\label{sec:derivation-mpm}

The aim of this section is to provide an overview of the standard
explicit \acrshort{mpm} algorithm~\cite{Sulsky1994}. In consequence it
...
restrictions that must be satisfied. First (i) the \textit{compatibility
  equation} that extracts from \gls{u} the strain field \gls{strain} is,
\begin{equation}
...
\gls{stress}, which satisfies (ii) the \textit{conservation of
  momentum equation}
\begin{equation}
...
FEM, in \acrshort{mpm} a double discretization procedure is performed as we will
...
\end{equation}
In \acrshort{mpm} literature, this equations
(\ref{eq:particle_balance_forces3}) and (\ref{eq:Updated_Lagrangian}),
are solved with an explicit forward Euler algorithm. The time integration scheme of \acrshort{mpm} has been described in detail by many researchers
...
\end{equation*}
As \acrshort{mpm} has a nodal stage, it is possible to apply this methods
successfully in \acrshort{mpm} framework as was proved by
...
in the context of FEM. In \acrshort{mpm} Gauss quadrature is not employed as
...
Ortiz (2006)\cite{Arroyo2006} has been recently tested under \acrshort{mpm} framework
...
considerably more accurate stress approximations for \acrshort{mpm}. Although, in
...
an estimate is to interpret the shape function $N_I(\vec{x})$ as a probability. This allow us to introduce two important limits:
...
\begin{align*}
  ...
  \end{cases}
\end{align*}
On the other hand, we wish to control the width of the shape function and it dacay with distance away from the corresponding nodes. To reach to this objective \cite{Arroyo2006} propose the following linear program,
\begin{align*}
  \label{eq:RAJAN}
...
find the neighbour nodes of a given integration point. An aditional remark is that in similariti  to alternative non-polynomial meshfree basis functions, the LME approximation scheme requires more than $d+1$ nodes to determine the values of the shape functions as well as their derivatives at any point in the convex hull of the nodal set, where $d$ is the dimension of the problem.
...
to those employed in the smoothed particle hydrodynamics (SPH), for
...
are compared with those provided by \acrfull{ugimp} and Q4 shape functions, also
we compare \acrshort{mpm} performance with OTM and FEM. All simulations were
...
\acrshort{ugimp}~\cite{Bardenhagen2004} shape function \textit{versus} the LME approximation scheme with
...
millimeters. During these simulation the \acrshort{ugimp} suffered an unstable
...
with OTM~\cite{Li2010} \textit{versus} the solution obtained with \acrshort{mpm},
both with same time integration scheme and spatial discretization. For
this case the performance of \acrshort{mpm} is robust and stable than OTM. As we
...
out to be unstable and in the second 15 it fails. The \acrshort{ugimp} simulation
...
employed as an useful technique in \acrshort{mpm}. The PCE arise as a highly
...
developed originally for FEM, which can be rearranged to \acrshort{mpm} framework
...
%%% TeX-master: t
%%% End:
\message{ !name(LME-MPM.tex) !offset(-1652) }
