...
\begin{abstract}
  ...
  integration scheme has been proposed. Its \textcolor{red}{powerful} \cancel{superior} performance
...
filtering them or increasing \red{somehow} the accuracy. \cancel{of the time integration
...
cross the cell boundaries. \red{Solving this issue is the main goal of the employment of the \acrshort{LME} shape functions}.
...
(2020)\cite{Wobbes2020}. \red{The performed simulations of \acrshort{mpm}
...
10$^{-6}$ \red{in this research} \cancel{for example},  would ensure a reasonable range of
...
conditions. Motivated by their increased extents, particles may share an influence radius that lies outside of the simulation domain. Some researchers have solved this problem with the so called ``extra'' or ``ghost'' nodes \cancel{by other investigators}. These nodes require especial treatment, similar
...
shape function. A proof of this statements is observed in figure~\ref{fig:LME_MPM}\red{COMENTAR MAS; ES LA DIFERENCIA CON WOBBES}. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure*}
  \centering
  \subfigure[Q4]{
...
tensile instability. \red{This is another benefit of the proposed methodology, that, although is out of the scope of the
...
This benchmark was proposed \red{by Dyka~\cite{Dyka1995} since allows so study easily the capability}
...
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In this case, \cancel{a duration of } 0.0001 seconds \red{are considered }in the simulation. Therefore, the elastic wave generated travel through the bar
...
of each particle. \red{In Fig. \ref{fig:Dyka-error-evol} the evolution of the RMS is obtained for both time integration schemes. The right figure, with the \acrshort{NPC} results, shows lower values of the estimated error, denoting the higher performance of this methodology. About the spatial discretization, the \acrshort{LME} schemes show an error comparable to the obtained with the \acrshort{uGIMP}, being even lower close to the grid-crossing region.}
...
simulation becomes unstable after $6E^{5}$ seconds. \cancel{Figure
...
solution sensibility depending on the regularization parameters, \red{is the behaviour of the solution depending on the decreasing of mesh
...
is \acrshort{fem}-like; therefore, larger values of $\gamma$ \red{may offer better results}. On the other hand, when mesh size is small enough to produce
grid-crossing meshfree behaviour is required to ensure the
convergence of the solution and tiny values of $\gamma$ \red{may lead to better
...
values of $\gamma$, \red{where the performance is punished with significant
...
millimeters. \red{The reason is due to an unstable
...
this phenomenon. \red{Conversely, this shortcoming is not suffered by \acrshort{lme}, independently of regular or with irregular nodal layout.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}\sidecaption
  \centering
  \resizebox{0.9\hsize}{!}{
