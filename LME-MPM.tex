...
\cite{Wang_2016} downsized these spurious noises by adding a non-viscous
...
\PNA{The proposed research has dived into the performance of the \acrfull{mpm} within dynamic problems of very fast loading rate. The main goal, which was the enhancement of such a methodology with different time and spatial discretization tools in order to stabilize the solution, has been satisfactory fulfilled.} 

\PNA{On the one hand,} a novel time integration scheme for
\acrshort{mpm} has been carried out. The \acrshort{npc} arises as a highly efficient
alternative for demanding dynamic problems \PNA{with high frequency of the applied loads. In the literature, several approaches of the \acrshort{npc} are proposed in order to compete on accuracy with expensive implicit time integration algorithms.} \PNA{The main achievement of the proposed time discretization has been observed within the Dyka's bar \ref{sec:dyka-bar}, obtaining lower values of the error than the ones obtained with the \acrfull{fe} scheme.}
Also,
...
originally for \acrshort{fem}, which can be rearranged to the
\acrshort{mpm} framework with the required modifications. \PNA{As well as the implementation of enhanced time integration schemes, further
research may be done in the comprehension of the
good performance of the algorithm within non-linear solutions, such as the coupling between different phases or the non-linear constitutive behaviour of the materials: the employment of this scheme to strengthen the localization capabilities of \acrshort{mpm} for viscoplastic materials is on the scope of this research line. The comparison with traditional time schemes in the non-linear field will improve the formal understanding of the proposed methodology.}
...
\acrshort{mpm} field, in order }to overcome grid crossing limitations and to avoid the
