...
frequency loads under the context of \acrshort{mpm}. \MODIFIED{Three different test have
...
Andersen (2009)\cite{thesis_Andersen_2009} and the classical example of two colliding discs provided by Sulsky {\it et al.} (1994) \cite{Sulsky1994}. The first example is devoted to test the accuracy of the \acrfull{npc} scheme is compared to the standard \acrfull{fe} in order to assess the performance of the time integration schemes. In the second example,  \acrshort{lme} solutions
...
\MODIFIED{\subsection{Impact of two elastic discs}}
\label{sec:Impact-two-elastic-discs}

\MODIFIED{The following test is best suited to validate the ability of the discussed
interpolation technique to deal with grid crossing instabilities. Also, it enlightens the energy conservation properties of the integration scheme, since it should remains constant before and after collision. The test proposed by Sulsky {\it et al.} (1994) \cite{Sulsky1994} consists in the impact of two identical elastic discs with the following elastic parameters :
\begin{itemize} 
...
\MODIFIED{The discs starts in the lower left and upper right corners with initial velocities of $(1, 1)$ and $(-1, -1)$, respectively. The computational domain consists in a square of 10 meters. It is discretised with a cartesian grid with a meshsize of 0.1. The courant number is fixed to 0.1 during the whole simulation. Details of the problem are sketched in figure~\ref{fig:Sulsky_discs}.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}
%\sidecaption
 \centering
 \resizebox{0.7\hsize}{!}{
   \input{Figures/Sulsky_discs.tex}}
 \caption{Geometrical description of the two colliding discs problem.}
 \label{fig:Sulsky_discs}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\MODIFIED{Energy plots in figure~\ref{fig:Sulsky-energy} proofs in one hand the ability of the \acrshort{lme} approximation to mitigate the grid-crossing error as well as the \acrshort{ugimp} interpolants, but without some  of its shortcomings previously mentioned in this paper. And in the other hand the energy conservation is also guaranteed with the \acrshort{npc} time integration scheme, unlike what happens in the \acrfull{usf} and \acrfull{usl} schemes. Although during impact mechanical energy decreases, no real dissipation occurs since after collision the value is again 5 Jules like before collision. Furthermore, kinetic energy has been exactly computed. Therefore, this apparently dissipation of mechanical energy is due to a numerical cancellation during the computation of the mechanical energy during post-process stage. }
...
%\sidecaption
  \centering
  \resizebox{\hsize}{!}{
    \includegraphics[width=\textwidth]{./Figures/Sulsky_energy}
  }
  \caption{Energy plots for different interpolation/approximation techniques. All simulations where performed with the already tested \acrshort{npc} algorithm. The evolution of the total energy of the system is represented with the solid line, the dashed line shows the evolution of the deformation energy, while the doted line represents the evolution of the kinetic energy.} 
  \label{fig:Sulsky-energy}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
