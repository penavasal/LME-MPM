...
block. \textcolor{green}{This figure shows how the \acrshort{mpm} simulation performed with a bi-linear interpolation technique (Q4) turns out to be unstable during cell-crossing and consequently fail}. \textcolor{green}{The \acrshort{lme} simulation was performed using two kinds of shape functions, one with a low value of the dimensionless parameter, $\widehat{\gamma} = 0.8$, and other with a larger value of it, $\widehat{\gamma} = 3.0$. Both simulations behaves with minimal oscillations, also not appreciable difference is observed between them. The \acrshort{ugimp} simulation is more stable than the one performed with the \acrshort{mpm}-Q4 and behaves like the \acrshort{lme} simulations. Finally, we have compared our results against two contrasted techniques. The \acrshort{fem} solution has been done with a regular mesh of linear quadrilaterals, meanwhile the \acrshort{otm} solution has been done with \acrshort{lme} approximation. The final displacement in both solutions oscillates around a value of 0.59 m.} 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}
%\sidecaption
  \centering
...
\textcolor{blue}{The background set of nodes consists of a cartesian grid with a uniform gap between nodes of 1 m, and four material points per cell were considered. The courant number is fixed to 0.1 during the whole simulation and $\text{LME}_{3.0}$ shape functions where used. The square initially move horizontally through the grid at $\text{V}_0 = 1.0 m/s$ until it hits an obstacle modelled as a set of nodes with zero velocity prescribed. The collision produce stress waves and cause the square to bounce back. Snapshots of velocity field magnitude within the square are given in figure
~\ref{fig:Magnitude_velocity_impact_square}}. 
%%%%%%%%%%%%%%%%
\begin{figure}
%\sidecaption
  \centering
...
\end{figure}
%%%%%%%%%%%%%%%% 
\textcolor{blue}{At early times, \acrshort{npc} and \acrshort{fe} are similar. At later times, the \acrshort{fe} introduces significant numerical noise as compared with \acrshort{npc}. At the end of the simulation, the filtering process when \acrshort{npc} is used leads to a negligible loss of mechanical energy (about 0.1\%). Finally, let us consider an unstructured background mesh.}
%%%%%%%%%%%%%%%%
\begin{figure}
%\sidecaption
  \centering
...
\includegraphics[scale=0.8]{Figures/Velocity-structured-unstructured-lme-ugimp.png}
  ...
  \caption{Sensitive of \acrshort{ugimp} and \acrshort{lme} to the randomness in the layout of the background set of nodes. On the left side of the image we can see sketches of the two meshes that have been used. And on the right side, snapshots with the velocity magnitudes in each case. All the simulation where carried out with the proposed \acrshort{npc}.}
  \label{fig:Magnitude_velocity_impact_square_structured_unestructured}
\end{figure}
%%%%%%%%%%%%%%%%
\textcolor{blue}{ As we can see in figure~\ref{fig:Magnitude_velocity_impact_square_structured_unestructured}, the \acrshort{lme} approximation preserves the solution even though the mesh is not structured. Meanwhile, the \acrshort{ugimp} is not able to deal with the unstructured mesh as expected. It goes without saying that there is no point in using unstructured meshes in this type of situation. Despite this, the \acrshort{lme} approach shows remarkable robustness in dealing with an unstructured mesh. This is useful in more complicated computational domains where structured mesh are not possible.}
