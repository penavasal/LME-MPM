...
{
  name=$ \Gamma_n$,
  description={Natural or Neumann boundary conditions over $\partial\varOmega$}
}
...
\begin{abstract}
  ...
  propagation and \MP{in the large deformation regime} is still challenging
  under this approach due the incapability of the standard \acrshort{mpm} time
  integration scheme to filter spurious noises. \MP{To overcome it ,
    we propose in this paper an explicit Predictor-Corrector time
    integration scheme}. Its \PNA{powerful} performance mitigates the
  presence of spurious oscillations with minimal dissipation in high
  frequency problems. Other source of numerical noise in
  \acrshort{mpm} occurs when the material points cross computational grid boundaries, being \MP{caused} by
  ...
  overcome it, \MP{we propose in this work} the \acrfull{lme} approximation
...
\begin{highlights}
\item We have proposed a new and efficient time integration scheme for
  the \acrshort{mpm}.
\item The proposal of the \acrshort{lme} approximation technique
  under the \acrshort{mpm} framework as a robust substitute of the
  traditional shape function.
\item Our approach have clear benefits for fast dynamics simulation
  since reduces the presence of spurious oscillations without
  damp the solution. 
\end{highlights}
...
without suffer mesh distortion inaccuracies.\PNA{One of the main
  fields where this method is strong} \MP{in solid dynamics \PNA{since the original time integration scheme proposed was the \acrfull{fe}~\cite{Sulsky1994}, carried out in a explicit manner. However, in this type or problems, the main instabilities of the original \acrshort{mpm} are even marked.}
...
introduce severe inaccuracies as noticed \MP{by} Steffen {\it et al.}
...
in \MP{some particular special patterns}, the quality of the \acrshort{ctls} approximation \MP{decrease} locally.

This document adopts the \acrfull{lme}, or Local \textit{Max-Ent} approximates, as a robust substitute\PNA{of the aforementioned} shape functions in \acrshort{mpm}. First introduced by Arroyo \& Ortiz
...
recently, under \acrshort{mpm} framework, the work made by Wobbes {\it et
  al.}(2020)~\cite{Wobbes2020}.\PNA{The proposed research delves into the benefits of
...
filtering them or increasing\PNA{somehow} the accuracy.\MMP{One of the first attempts was
the proposal of an implicit time integration scheme by Guilkey \&
Weiss \cite{Guilkey_2003}. More recently Wang {\it et al.}
\cite{Wang_2016}} \MP{mitigated} \MMP{these spurious noises by adding a non-viscous
...
Kularathna \& Soga \cite{Soga_2017} study an implicit treatment of the pressure in MPM algorithm to simulate material incompressibility
...
effort.} In this paper a less time consuming and high efficient explicit predictor-corrector integration method \MP{is}
proposed. It consists of an accommodation of the traditional \acrsull{npc} scheme, widely employed in Finite Element methods. This method
...
generality, the method \MP{consists of} three main steps: (i) a
variational recovery process, where particle data is projected \MP{onto} the
...
In \acrshort{mpm} the continuum mechanics approach is considered. So on, \MP{we will consider a region \gls{domain} }occupied by an elastic body like
...
restrictions that must be satisfied.\MP{First (i) we will relate
...
\gls{dirichlet-boundary}.\MP{We will further assume that strains are
infinitesimal}, therefore second order terms
in the spatial derivatives can be neglected.\MP{From here}, \PNAthe
stress field \gls{stress} will be considered the corresponding
conjugate variable for the strain field, being the one} which
satisfies (ii) the \textit{conservation of \MP{lineal} momentum equation}:
\begin{equation}
...
application \MP{from $\mathbb{R}^n$ to $\mathbb{R}^n$, which relate stress and strain increments as,}
\begin{equation}
  \label{eq:Constitutive-equation}
\tens{\sigma} = \tens{D} \colon \tens{\varepsilon}.
\end{equation}
\PNA{In this research, plane strain Linear Elasticity has been
  considered. Thus the constitutive tensor, \tens{D}, is the well
  known linear elastic one.} The final \MP{equation of the set} is (iv) the mass
...
\end{equation}
\MP{in which} the Cauchy sequences are convergent in \gls{domain} as well:
\begin{equation}
...
\mathcal{H}^1_0$ such that, for \MP{any} $\vect{u}^{\psi} \in
\mathcal{H}^1_0$,
the following holds:
\begin{equation}
...
\MP{Next, we will discretize the set of equations of the mathematical model using a double discretization procedure.} First, the continuum \gls{domain} is discretized with a finite sum of material points (also denominated particles the manuscript). Each material point
...
virtual displacement field $\vect{u}^{\psi}_{p}$. \MP{Regarding numerical integration, the material derivatives are
...
\end{equation}
\end{itemize}
where $h$ is the thickness of the continuum in a 2D case. \MP{Next, we
...
\end{equation}
Finally, to solve the equation \eqref{eq:particle_balance_forces3}, a second order\PNA{time} integration scheme is required. Therefore, time is
...
$\tens{m}$. This issue may lead to numerical \MP{problems} when nodal acceleration
...
explicit scheme, which is also \MP{referred to as} a-form 
...
\MP{within} \PNA{the \acrshort{mpm} framework in a similar manner that the one
proposed by Tran~\it{et al.}~\cite{Tran2019e}.} \MP{By using the} \PNAthe predictor definition, it is possible to calculate nodal velocities and update particles position employing nodal values
...
\textit{corrector} stage is introduced. \MP{As} nodal
...
\end{equation}
Finally updated particle \MP{accelerations, velocities and positions are updated as},
\begin{align}
...
with the \textit{leapfrog \MP{scheme}} where position is not updated at
...
(2020)\cite{Wobbes2020}. \MP{The simulations presented in \cite{Wobbes2020} of \acrshort{mpm}
within \acrshort{lme} show considerably more accurate stress
approximations than traditional \acrshort{mpm} schemes.} \PNA{However, 
...
Due to the \short{fem}-compatibility, the \acrshort{lme} shape
...
number of dimensions in the problem. \MMP{Ullah {\it et al.}
  \cite{Augarde_2013} take advantage of the \acrshort{lme}
  \short{fem}-compatibility to couple the
  \acrfull{efgm} and \short{fem} for linear elasticity and for problems with both material and geometrical nonlinearities.}Furthermore, with a conveniently
adopted \textit{regularization} parameter it is possible to get a 
\acrshort{gimp}-like shape function.\MMP{Finally \short{sph}-like
...
out by using this technique. The employment of smoothing algorithm is
also straightforward in the fluid-solid interaction problems
\cite{Arduino_2018}.A proof of this statements is observed in
figure~\ref{fig:LME_MPM}, where the \acrshort{lme} adaptability is illustrate.}\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure*}
  \centering
  \subfigure[Q4]{    
  ...
  \subfigure[$\text{LME}_{17}$]{
    \begin{tabular}{c}      \includegraphics[width=0.14\textwidth]{Figures/LME_17_3_Shape_Fun}\\
...
\rvert_{x=L} = 0$) in the right border, \MP{being free on all other boundaries.} An initial velocity of $\vect{v}_o = - 5\ m/s$ is given to the
left quarter of the bar. Finally, the elastic parameters \MP{chosen} for this test are:
\begin{itemize} 
...
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\MP{In the proposed exmple, the simulation extends from time zero up to 0.00001 s.
...
\MP{located} the exact quadrature points of a linear quadrilateral, with
...
A first \MP{comparison} between the \MP{obtained in} both time integration
...
\begin{figure}\sidecaption
  ...
  \caption{\MP{comparison of \acrshort{npc} and \acrshort{fe}
...
Figure \eqref{fig:Dyka-LME-gamma} shows the \MP{sensitivity} of the \acrshort{lme}
...
extremely \MP{noisy} oscillations could damage the solutions like memory
materials. On the other hand larger values of the parameter $\beta$
make the solution \MP{to tend} to the linear \acrshort{fem} solution as the athermal limit is reached \cite{Arroyo2006}. Intermediate values of the
regularization parameters give us a compromise \MP{between both scenarios described here}. An additional observation, concerning to the
...
\begin{figure}\sidecaption
  ...
  \caption{\MP{Sensitivity} of LME approximants performance to changes in the
...
\MP{We have proposed in this paper an improved methodology which improves \acrfull{mpm} behaviour in fast dynamics problems. Different time and space discretization techniques have been presented, their performance being compared agains other approaches in two benchmark problems.}
\MP{First,} a novel time integration scheme for
\acrshort{mpm} has been carried out. The \acrshort{npc} \MP{has been
  shown to be a highly} efficient alternative for demanding \MP{fast dynamics} problems. \PNA{In the literature, several approaches of the \acrshort{npc} are proposed in order to compete on accuracy with expensive implicit time integration algorithms.} \PNA{The main achievement of the proposed time discretization has been observed within the Dyka's bar \ref{sec:dyka-bar}, obtaining lower values of the error than the ones obtained with the \acrfull{fe} scheme.}
...
originally for \acrshort{fem}, which can be \MP{implemented within a
  \acrshort{mpm} framework with the suitable} modifications. \MP{In
addition to} \PNA{the implementation of enhanced time integration schemes, further
research} \MP{can be done in the analysis of the good performance} \PNA{of the algorithm within non-linear solutions, such as the coupling between different phases or the non-linear constitutive behaviour of the materials: the employment of this scheme to strengthen the localization capabilities of \acrshort{mpm} for viscoplastic materials is on the scope of this research line. The comparison with traditional time schemes in the non-linear field will improve the formal understanding of the proposed methodology.}

\PNA{On the other hand, the spatial discretization of the \acrshort{mpm} is enhanced thanks to the powerful skills of the \acrfull{lme} approximation scheme, which has been validated as a robust and versatile
tool in the \acrshort{mpm} framework.} It also comes up as a promising
alternative to other approximation techniques\PNA{, developed within the
\acrshort{mpm}} \MP{framework, in order }to overcome grid crossing limitations and to avoid the
constriction of the \acrshort{ugimp} of a regular mesh or a high
density of particles per cell. \PNA{Several research lines of the \acrshort{lme} scheme dive into the improvement of the methodology, focusing in the optimization of the calculation of the shape function and the possibility of adapting the parameter $\beta(\vect{x})$ through the finite strain
...
region. \PNA{Together with the implementation of non-linear behaviours, previously mentioned, challenging
