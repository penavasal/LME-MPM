...
The subscript $p$ is used to define a particle variable. While the
subscript $I$ is reserved in this notation for denoting nodal
variables.

The superscript $\psi$ involves a virtual magnitude.

$\dot{\qed}$ 
where $\Div{\qed}$ denotes the divergence operator.

where $\GradS{\qed}$ denotes the symmetric part of the gradient
operation. 

$\varGamma_d$  $\varGamma_n$
...
\emptyset$. The behaviour of the continuum can be described by a set
of governing equations which are (i) the balance of momentum equation
\begin{equation}
...
\begin{equation}
  ...
  \frac{D \rho}{D t} = \dot{\rho} + \rho \Grad{\vect{v}} = 0.
\end{equation}
...
\item Internal forces :
  \begin{equation}
   ...
   \tens{\sigma}_{p}\ \colon \Grad{\tens{\psi}_{p}}\ V_p.
  \end{equation}\\
\item Body forces :
\begin{equation}
...
\begin{equation}
  ...
  \vec{v}_{p} = \sum^{Nn}_I N_I(\vec{x}_p)\cdot \vect{v}_I
  \quad \text{and} \quad \vec{v}^{\psi}_{p} = \sum^{Nn}_I N_I(\vec{x}_p)\cdot \vect{1},
\end{equation}
...
\begin{equation}
  ...
  \vec{f}_{I}^{int} = - \sum^{N_p}_{p} \tens{\sigma}_{p} \colon \Grad{N_{Ip}} \frac{m_p}{\rho_p}
\end{equation}
\begin{equation}
  \label{eq:nodal_external_forces}
  \vec{f}_{I}^{ext} = \sum^{N_p}_{p} \vec{b}_{p} N_{Ip} m_p  + \sum^{N_p}_{p}
