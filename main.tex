...
\begin{abstract}
  ...
  frequency problems. Other source of numerical noise in the MPM 
...
introduce severe inaccuracies. This is similar to the difficulty
...
been introduced in to the MPM framework with the Conservative Taylor
...
This document adopts the \textit{local maximum-entropy approximation
  ...
  function in the MPM. First introduced by Arroyo \& Ortiz
...
for the MPM with promising results but at the expense of increasing
...
particles. In consequence the MPM can be regarded as a
...
define a continuum domain $\varOmega$ occupied by an elastic body like the sketched in
\ref{fig:Continuum-solid}, and $\partial \varOmega$ the boundaries of
the domain defined by $\partial \varOmega = \Gamma_d \bigcup
\Gamma_n$ and $\Gamma_d \cap  \Gamma_n = \emptyset$.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{figure}\sidecaption
  ...
  \caption{Description of the MPM standard algorithm.}
...
To accommodate this step to the MPM framework, it is necessary to get
...
pseudo-algorithm \ref{app:explicit-predictor-algorithm}.
...
Ortiz (2006)\cite{Arroyo2006} has been recently tested under the MPM framework
...
shape functions in the MPM is exposed. To avoid the transference of
...
those particles close to the boundary is required. Here we employ an
algorithm inspired in the one proposed by Kumar {\it et al.}
(2019)\cite{Kochmann2019} to deal with interfaces between two
materials in the OTM framework. It consists in select only those nodes

employed to compute the solution are those whose
position are inside 
In opposite to the OTM where
this approximation technique has been widely.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}\sidecaption
  \centering
  \resizebox{0.7\hsize}{!}{
  ...
  \caption{This tolerance defines the limit values of the influence
    radius and is used thereafter to find the neighbour nodes of a given integration point.} 
...
shape function. A proof of this statements is observed in \ref{fig:LME_MPM}.  
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure*}
  \centering
  \subfigure[Q4]{
...
\label{app:explicit-predictor-algorithm}
